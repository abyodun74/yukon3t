 ...
 * (no circleId) or for a PUBLIC Circle is open to any signed-in user — the
 * Home "Live now" strip lists both. A stream started for a PRIVATE Circle or
 * sub-circle is for THAT Circle's members only — not the host's followers, not
 * a Home feed, not anyone who merely has the stream's id (from a notification,
 * a shared link, or a guess). The host and site admins (who moderate) are
 * always allowed.
 *
