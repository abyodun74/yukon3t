...
- **GIF attachments (messages/posts/comments) skip this app's own moderation pipeline entirely**, trusting Tenor's pre-moderated catalog instead — deliberate, since Tenor's search API returns no way to fetch the underlying file for our own OpenAI/Hive moderation to inspect, and re-hosting every result through R2 first just to moderate it would add real cost/latency for content already moderated upstream. `isTenorUrl` (`src/lib/tenor.ts`) is the only gate: it validates the attached URL's host is actually `tenor.com`/`*.tenor.com` before accepting it, which is what stops a client from smuggling an arbitrary external URL through this path — never widen that check to a generic `https:` allowlist.
