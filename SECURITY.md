...
4. Promote the first real admin account manually (`UPDATE "User" SET "isAdmin" = true WHERE email = '...'`) — there is no self-serve admin signup path by design. Not yet done on the live Neon database as of this deploy.
5. **Update the R2 bucket's CORS policy** to include the production origin (`https://yukon3t.vercel.app`), not just `http://localhost:3000` — media uploads (avatar/photo/video) will fail with a CORS error on the live site until this is added via the Cloudflare dashboard (not something available through the S3-compatible API this app's credentials are scoped to). Not yet done as of this deploy.
6. `OPENAI_API_KEY` is still unset on production by explicit choice — moderation is in fail-open mode on the live site right now. Add it before real users start posting.
