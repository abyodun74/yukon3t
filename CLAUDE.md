...
NextAuth v5 (Resend magic link) is the default, but `src/app/actions/password-auth.ts` implements a **fully custom** username/password path — not NextAuth's Credentials provider, because Credentials doesn't support the `"database"` session strategy this app uses. It writes directly to the same `Session` table and sets the identical cookie NextAuth would (`src/lib/auth-cookie.ts`). Both paths converge on the same `auth()` calls everywhere else. If you touch session/cookie logic, both paths need to keep working.
